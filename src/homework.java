...
public class homework {
    ...
    static boolean isWinner(char symb, int endOf)
    {
        int endOfset = map.length - DOTS_TO_WIN;

        for (int rowOffset = 0; rowOffset <= endOfset; rowOffset++)
        {
            if (isWinner(symb, rowOffset))
            {
                return true;
            }

            for (int columnOffset = 0; columnOffset <= endOfset; columnOffset++) {
                boolean hasWin =
